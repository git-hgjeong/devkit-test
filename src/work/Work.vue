		...
		<section id="container" class="container">
			...
			<section class="content CI-MDI-WRAP" style="">
				...
				<ul class="tab1 CI-MDI-TAB-WRAP" id="jsMdiTab">
					<li v-for="(item, index) in midMenuList" v-bind:key="item" v-bind:class="item.tabClass"><a href="javascript:void(0);" @click="onClickMdiTap(item, index)"  v-bind:title="'['+item.mid+']'+' '+item.title" class="CI-MDI-TAB-NAME"><span>{{item.name}}</span><button type="button" title="닫기" class="CI-MDI-TAB-CLOSE"></button></a></li>
				</ul>
